...
// ✅ Corrected createOptimization function
exports.createOptimization = async (req, res) => {
  ...
  try {
    ...
    if (locationIds.length > 100)
      return res.status(400).json({ msg: 'Maximum 100 locations allowed for optimization.' });

    if (vehicleIds.length > 20)
      return res.status(400).json({ msg: 'Maximum 20 vehicles allowed for optimization.' });

    // ================== FETCH DATA ==================
    const vehicles = await Vehicle.find({ _id: { $in: vehicleIds }, user: req.user.id });
    const locations = await Location.find({ _id: { $in: locationIds }, user: req.user.id });

    if (vehicles.length === 0)
      return res.status(400).json({ msg: 'No valid vehicles found. Check your selection.' });

    if (locations.length === 0)
      return res.status(400).json({ msg: 'No valid locations found. Check your selection.' });
    ...
    const SPEED_KMH = 40; // ✅ Default speed constant for travel duration
    ...
    if (runComparison) {
      ...
      for (const [key, algo] of Object.entries(algorithmMap)) {
        try {
          ...
          // Run algorithm (returns raw routes)
          const rawRoutes = await Promise.resolve(algo.function(vehicles, locations, depot));
          ...
          algorithmResults.push({
            ...
            averageRouteDistance: rawRoutes.length ? totalDistance / rawRoutes.length : 0,
            averageRouteDuration: rawRoutes.length ? totalDuration / rawRoutes.length : 0
          });
        } catch (algoErr) {
      ...
      // ✅ Clean and assign vehicles for all algorithms
      console.log('Assigning vehicles to all comparison results...');
      algorithmResults.forEach(result => {
        if (result.routes && !result.error) {
          result.routes = assignVehiclesToRoutes(result.routes, vehicles, {}, SPEED_KMH);
        }
      });

      // ✅ Select best algorithm (max coverage, then min distance)
      ...
      if (validResults.length > 0) {
        ...
        bestResult = bestCandidates.reduce((best, current) =>
    ...
    try {
      const startTime = Date.now();
      const rawRoutes = await Promise.resolve(selectedAlgo.function(vehicles, locations, depot));
      const routesWithVehicles = assignVehiclesToRoutes(rawRoutes, vehicles, {}, SPEED_KMH);
      const executionTime = Date.now() - startTime;
...
// Corrected Simulated Annealing VRP Algorithm
// Corrected Simulated Annealing VRP Algorithm
function simulatedAnnealingVRP(vehicles, locations, depot) {
  console.log("Running Simulated Annealing VRP...");
  const speedKmh = 40;
  
  // Build the distance matrix for cost calculations
  const distances = {};
  locations.forEach((l1) => {
  ...
  // --- 1. Generate a good initial solution ---
  let currentSolution = enhancedClarkeWrightAlgorithm(vehicles, locations, depot);
  let bestSolution = JSON.parse(JSON.stringify(currentSolution));
  ...
  // Create a map for quick vehicle capacity lookups
  const vehicleCapacityMap = new Map(vehicles.map(v => [v._id.toString(), v.capacity || 0]));

  // --- Helper function to generate a valid neighbor solution ---
  const generateNeighbor = (solution) => {
    ...
    // 50% chance for an inter-route move
    if (moveType < 0.5 && neighbor.length >= 2) {
      // --- Inter-Route Relocation (with capacity check) ---
      let routeAIndex = Math.floor(Math.random() * neighbor.length);
      let routeBIndex = Math.floor(Math.random() * neighbor.length);
      
      const routeA = neighbor[routeAIndex];
      const routeB = neighbor[routeBIndex];

      // Ensure we have a valid source and destination
      if (routeAIndex === routeBIndex || routeA.stops.length <= 2) {
        return neighbor; // Not a valid move, try again
      }

      // ========================== ERROR FIX START ==========================
      // A destination route MUST have an assigned vehicle to receive a stop.
      // If `routeB.vehicle` is undefined, we cannot perform the move.
      if (!routeB.vehicle) {
        // This is not a valid destination, so we return the unmodified neighbor
        // and let the algorithm try a different move in the next iteration.
        return neighbor;
      }
      // =========================== ERROR FIX END ===========================

      const stopIndexToMove = Math.floor(Math.random() * (routeA.stops.length - 2)) + 1;
      const stopToMove = routeA.stops[stopIndexToMove];

      // VALIDATION: Check if the destination vehicle has enough capacity
      const routeBCapacity = vehicleCapacityMap.get(routeB.vehicle.toString()) || 0;
      if ((routeB.totalCapacity || 0) + (stopToMove.demand || 0) <= routeBCapacity) {
        const [movedStop] = routeA.stops.splice(stopIndexToMove, 1);
        routeA.totalCapacity -= movedStop.demand || 0;

        const insertIndex = Math.floor(Math.random() * (routeB.stops.length - 1)) + 1;
        routeB.stops.splice(insertIndex, 0, movedStop);
        routeB.totalCapacity += movedStop.demand || 0;
        
        if (routeA.stops.length <= 2) {
            neighbor.splice(routeAIndex, 1);
        }
      }
      ...
      if (route.stops.length >= 4) {
        const i = Math.floor(Math.random() * (route.stops.length - 2)) + 1;
        const j = Math.floor(Math.random() * (route.stops.length - 2)) + 1;
...
function antColonyOptimizationVRP(vehicles, locations, depot) {
  ...
  const beta = 3.0;  // Heuristic (distance) importance
  const rho = 0.2;   // Pheromone evaporation rate
  ...
  locations.forEach(l1 => {
    ...
    locations.forEach(l2 => {
      const id2 = toId(l2._id);
      pheromones[id1][id2] = 1.0; // Initial pheromone level
    });
  });
  ...
  for (let iter = 0; iter < numIterations; iter++) {
    ...
    for (let ant = 0; ant < numAnts; ant++) {
      const solution = constructSolutionForAnt(vehicles, locations, depot, distances, pheromones, alpha, beta);
      ...
      if (cost < bestCost) {
        ...
        console.log(`New best solution found in iteration ${iter}, ant ${ant}: Cost ${bestCost.toFixed(2)}`);
      }
    }
    ...
    // Deposition (using the best solution of the iteration)
    const iterBest = allAntSolutions.reduce((prev, curr) => (prev.cost < curr.cost ? prev : curr));
    if (iterBest && iterBest.cost !== Infinity) {
      const deltaPheromone = Q / iterBest.cost;
  ...
  console.log(`ACO finished. Best cost found: ${bestCost.toFixed(2)}`);
  return bestSolution || [];
}

// Helper for an ant to construct a complete solution (set of routes)
function constructSolutionForAnt(vehicles, locations, depot, distances, pheromones, alpha, beta) {
  const toId = (objId) => objId.toString();
  const depotId = toId(depot._id);
  const speedKmh = 40;

  const unvisited = new Set(locations.filter(l => toId(l._id) !== depotId).map(l => l._id));
  const routes = [];
  ...
  for (const vehicle of vehicleSlots) {
    ...
    while (unvisited.size > 0) {
      const nextLocation = chooseNextLocation(currentLocation, unvisited, locations, vehicle.capacity, currentCapacity, distances, pheromones, alpha, beta);

      if (!nextLocation) {
        break; // No feasible location found for this vehicle
      }

      // Add location to route
      currentCapacity += nextLocation.demand || 0;
      route.stops.push({
        locationId: nextLocation._id, locationName: nextLocation.name, latitude: nextLocation.latitude, longitude: nextLocation.longitude, demand: nextLocation.demand || 0, order: route.stops.length
      });
      unvisited.delete(nextLocation._id);
      currentLocation = nextLocation;
    }

    // End route at the depot
    route.stops.push({
      locationId: depot._id, locationName: depot.name, latitude: depot.latitude, longitude: depot.longitude, demand: 0, order: route.stops.length
    });

    if (route.stops.length > 2) { // If the route visited at least one location
      // Recalculate final route metrics
      let totalDist = 0;
      for (let i = 0; i < route.stops.length - 1; i++) {
          const fromId = toId(route.stops[i].locationId);
          const toIdStr = toId(route.stops[i + 1].locationId);
          totalDist += distances[fromId][toIdStr] || 0;
      }
      route.distance = totalDist;
      route.duration = Math.round((totalDist / speedKmh) * 60);
...
// Probabilistic choice for the next location
function chooseNextLocation(current, unvisited, allLocations, vehicleCapacity, currentLoad, distances, pheromones, alpha, beta) {
  const toId = (objId) => objId.toString();
  const currentLocationId = toId(current._id);
  ...
  unvisited.forEach(locationId => {
    ...
    if (candidate && (currentLoad + (candidate.demand || 0)) <= vehicleCapacity) {
      const candidateId = toId(candidate._id);
      const distance = distances[currentLocationId][candidateId] || 1; // Avoid division by zero
      const pheromone = pheromones[currentLocationId][candidateId] || 1;
      const heuristic = 1 / distance;

      const probability = Math.pow(pheromone, alpha) * Math.pow(heuristic, beta);
      choices.push({ location: candidate, probability });
  ...
  return choices[choices.length - 1].location; // Fallback
}
...
//  const PYTHON_API_URL = 'https://backend-for-route-optimization-81mu.onrender.com';
 const PYTHON_API_URL = 'http://127.0.0.1:8000/optimize'; // URL of your running FastAPI app
  ...
  try {
    ...
    return clarkeWrightAlgorithm(vehicles, locations, depot);
  }
}
