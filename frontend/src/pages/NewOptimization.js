...
const handleOptimize = async () => {
  ...
  // --- NEW: Check total vehicle capacity vs total demand ---
  const totalVehicleCapacity = vehicles
    .filter(v => selectedVehicles.includes(v._id))
    .reduce((total, v) => total + ((v.capacity || 0) * (v.count || 1)), 0);

  const totalLocationDemand = locations
    .filter(l => selectedLocations.includes(l._id))
    .reduce((total, l) => total + (l.demand || 0), 0);

  if (totalVehicleCapacity < totalLocationDemand) {
    const errorMsg = `Selected vehicles do not have enough capacity to cover total demand. Vehicle capacity: ${totalVehicleCapacity}, Total demand: ${totalLocationDemand}`;
    setError(errorMsg);
    notify(errorMsg, 'error', { autoClose: 8000 });
    return;
  }
