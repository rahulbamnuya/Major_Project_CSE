...
// ================== HELPER FUNCTION ==================
/**
 * Converts minutes from midnight into a user-friendly "HH:mm" format.
 * @param {number | null | undefined} minutes - The time in minutes.
 * @returns {string} The formatted time or 'Anytime' as a fallback.
 */
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        {locations.length > 0 && (
          <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-2xl overflow-hidden mb-8">
            <div className="p-6 border-b border-slate-200 dark:border-slate-700">
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Interactive Map</h2>
              <p className="text-slate-600 dark:text-slate-400">View your locations on the map</p>
            </div>
        ...
        {locations.length === 0 ? (
          <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 p-12 text-center shadow-lg">
            ...
            <p className="text-slate-600 my-4">Add your first location to get started.</p>
            <Link to="/locations/add" className="btn btn-primary"><FaPlus className="mr-2" />Add Location</Link>
          </div>
        ) : (
          <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-2xl overflow-hidden">
            <div className="p-6 border-b border-slate-200 dark:border-slate-700">
              ...
              <p className="text-slate-600 dark:text-slate-400">Manage and edit your delivery locations</p>
            </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-700">
                    <th className="px-6 py-4 text-left text-sm font-semibold text-slate-900 dark:text-white uppercase tracking-wider">Location</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-slate-900 dark:text-white uppercase tracking-wider">Coordinates</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-slate-900 dark:text-white uppercase tracking-wider">Type</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-slate-900 dark:text-white uppercase tracking-wider">Demand</th>
                    {/* ================== UPDATED TABLE HEADER ================== */}
                    <th className="px-6 py-4 text-left text-sm font-semibold text-slate-900 dark:text-white uppercase tracking-wider">Time Window</th>
                    {/* ========================================================== */}
                    <th className="px-6 py-4 text-left text-sm font-semibold text-slate-900 dark:text-white uppercase tracking-wider">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                  {locations.map(location => (
                    <tr key={location._id} className="hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors duration-200">
                      <td className="px-6 py-4">
                        <div className="font-semibold text-slate-900 dark:text-white">{location.name}</div>
                        {location.address && <div className="text-sm text-slate-600 dark:text-slate-400">{location.address}</div>}
                      </td>
                      ...
                      <td className="px-6 py-4">
                        {location.isDepot
                          ? <span className="inline-flex items-center gap-2 px-3 py-1 bg-orange-100 dark:bg-orange-900 text-orange-800 dark:text-orange-200 rounded-full text-sm font-medium"><FaWarehouse />Depot</span>
                          : <span className="inline-flex items-center gap-2 px-3 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded-full text-sm font-medium"><FaMapPin />Delivery Point</span>}
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm font-medium text-slate-900 dark:text-white">{location.demand || 0}</span>
                      </td>
                      {/* ================== UPDATED TABLE CELL WITH ROBUST CHECK ================== */}
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex gap-2">
                          <Link to={`/locations/edit/${location._id}`} className="btn btn-secondary btn-sm"><FaEdit className="mr-1" />Edit</Link>
                          <button onClick={() => handleDelete(location._id)} className="btn btn-danger btn-sm"><FaTrash className="mr-1" />Delete</button>
                        </div>
                      </td>
                    </tr>
                  ))}
